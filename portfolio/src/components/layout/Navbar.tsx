import { useState, useEffect, AnchorHTMLAttributes, PropsWithChildren } from "react";
...
const Navbar = () => {
  // Lightweight local replacements for `wouter` to avoid dependency errors.
  // These use basic browser navigation and anchors.
  const Link = ({ href, children, ...props }: PropsWithChildren<AnchorHTMLAttributes<HTMLAnchorElement> & { href: string }>) => (
    <a href={href} {...props}>{children}</a>
  );

  const useLocation = () => {
    const [loc, setLoc] = useState(typeof window !== "undefined" ? window.location.pathname : "/");
    useEffect(() => {
