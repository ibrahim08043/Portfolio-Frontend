            ...
            <>
              ...
              <div className={`flex flex-wrap gap-2 mt-3 ${isEven ? 'justify-start md:justify-end' : ''}`}>
                {experience.skills.map((skill, idx) => (
                  <span 
                    key={idx} 
                    className={`text-xs px-2 py-1 bg-${experience.color} bg-opacity-20 text-white rounded-full`}
                  >
                    {skill}
                  </span>
                ))}
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {experience.skills.map((skill, idx) => (
                  <span 
                    key={idx} 
                    className={`text-xs px-2 py-1 bg-${experience.color} bg-opacity-20 text-black rounded-full`}
                  >
                    {skill}
                  </span>
                ))}
