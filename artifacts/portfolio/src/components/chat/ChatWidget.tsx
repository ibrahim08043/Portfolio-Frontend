  ...
  const sendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiRequest<{ response: string }>(
        ...
        { method: "POST", body: JSON.stringify({ message: userMessage.content, history }) }
      );
      ...
      setState((prev) => ({
        ...
        messages: [
          ...prev.messages,
          { role: "assistant", content: "Sorry, I couldn't process your request. Please try again later.", timestamp: new Date() },
        ],
        isLoading: false,
      }));
    }
  ...
  return (
    <div className="fixed bottom-7 right-7 z-50">
      ...
      <AnimatePresence>
        {state.isOpen && (
          <motion.div
            className="absolute bottom-16 right-0 w-80 md:w-96 h-[400px] flex flex-col overflow-hidden
